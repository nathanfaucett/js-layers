...
tape("Router#handler(req : Request, res : Response[, callback(err : Error, req, res) : Function]) should call all routes/middleware based on req.pathname or url.parse(req.url).pathname", function(assert) {
    ...
    router.handler({
        ...
        function() {
            assert.equal(calledMiddleware, true, "should call all middleware");
            assert.equal(calledRoute, true, "should call routes");
        }
    );
    ...
    router.handler({
        ...
        function(err) {
            assert.equal(err.message, "404 - Not Found", "should return 404");
        }
    );
